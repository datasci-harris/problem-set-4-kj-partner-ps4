...
```
3. 
184 zip codes were indirectly affected.
```{python}
...
direct_and_indirect_closures_choropleth = alt.Chart(alt.Data(values=texas_geojson['features'])).mark_geoshape().encode(
    color=alt.Color(
    ...
    tooltip=[
        alt.Tooltip('properties.ZIP_CD_left:O', title='ZIP Code'),  # Check that this matches your data
...
direct_and_indirect_closures_choropleth
```
