...
```
    ...
    b.
    My calculation shows that the average distance from a Texas zipcode's centroid to a hospital in Texas is 317.04 miles. This does not make sense.
```{python}
...
4. 
```{python}
# Convert the joined_buffered_gdf to GeoJSON for visualization
texas_geojson = json.loads(joined_buffered_gdf.to_json())

# Create the choropleth map
choropleth = alt.Chart(alt.Data(values=texas_geojson['features'])).mark_geoshape().encode(
    color=alt.Color(
        'properties.Affected_Status:N',
        scale=alt.Scale(
            domain=['Directly Affected', 'Indirectly Affected', 'Not Affected'],
            range=['red', 'orange', 'lightgray']
        ),
        legend=alt.Legend(title='Affected Status')
    ),
    tooltip=[
        alt.Tooltip('properties.ZIP_CD:O', title='ZIP Code'),
        alt.Tooltip('properties.Affected_Status:N', title='Affected Status')
    ]
).properties(
    width=600,
    height=400,
...
# Load shapefile and filter for ZIP codes in Texas
shapefile = gpd.read_file(shapefile_path)
shapefile['ZCTA5'] = shapefile['ZCTA5'].astype(str)
texas_zips = shapefile[shapefile['ZCTA5'].str.startswith(('75', '76', '77', '78', '79'))]

# Set a consistent CRS for distance calculations
texas_zips = texas_zips.to_crs(epsg=32614)

# Load, filter, and compile closure data from each year
closure_data = {}
for year, file in data_files.items():
    yearly_data = pd.read_csv(file, encoding='ISO-8859-1', low_memory=False)
    filtered_data = yearly_data[yearly_data['PGM_TRMNTN_CD'].isin([1, 2, 3, 4, 5, 6, 7])]
    filtered_data['Year'] = year
    closure_data[year] = filtered_data[['PRVDR_NUM', 'PGM_TRMNTN_CD', 'ZIP_CD', 'Year']]

# Combine data and count closures per ZIP code
total_closures = pd.concat(closure_data.values())
closure_counts = total_closures['ZIP_CD'].value_counts().reset_index()
closure_counts.columns = ['ZIP_CD', 'closure_count']

# Identify directly affected ZIP codes
directly_affected = closure_counts[closure_counts['closure_count'] > 0]
directly_affected['ZIP_CD'] = directly_affected['ZIP_CD'].apply(lambda x: str(int(float(x))))

# Create GeoDataFrame for directly affected ZIP codes
direct_gdf = texas_zips[texas_zips['ZCTA5'].isin(directly_affected['ZIP_CD'])].copy()
direct_gdf['category'] = 'Directly Affected'

# Create a 10-mile buffer around directly affected ZIP codes
direct_gdf['geometry'] = direct_gdf.geometry.buffer(10 * 1609.34)

# Find indirectly affected ZIP codes via spatial join
indirect_gdf = gpd.sjoin(texas_zips, direct_gdf, how='inner', predicate='intersects')
indirect_gdf = indirect_gdf[~indirect_gdf['ZCTA5_left'].isin(direct_gdf['ZCTA5'])]
indirect_gdf['category'] = 'Indirectly Affected'

# Identify ZIP codes not affected
unaffected_gdf = texas_zips[~texas_zips['ZCTA5'].isin(directly_affected['ZIP_CD']) &
                            ~texas_zips['ZCTA5'].isin(indirect_gdf['ZCTA5_left'])]
unaffected_gdf['category'] = 'Not Affected'

# Combine all data into one GeoDataFrame
combined_gdf = pd.concat([
    direct_gdf[['ZCTA5', 'geometry', 'category']],
    indirect_gdf[['ZCTA5_left', 'geometry', 'category']].rename(columns={'ZCTA5_left': 'ZCTA5'}),
    unaffected_gdf[['ZCTA5', 'geometry', 'category']]
])

# Ensure consistency in CRS
combined_gdf = combined_gdf.to_crs(epsg=32614)

# Convert GeoDataFrame to GeoJSON for Altair
combined_geojson = json.loads(combined_gdf.to_json())

# Altair plot
TX_Closure_Impact = alt.Chart(alt.Data(values=combined_geojson['features'])).mark_geoshape().encode(
    color=alt.Color('properties.category:N',
                    scale=alt.Scale(domain=['Directly Affected', 'Indirectly Affected', 'Not Affected'],
                                    range=['red', 'orange', 'lightgray']),
                    legend=alt.Legend(title='Closure Impact')),
    tooltip=[
        alt.Tooltip('properties.ZCTA5:O', title='ZIP Code'),
        alt.Tooltip('properties.category:N', title='Category')
    ]
).properties(
    width=800,
    height=600,
    title='Texas ZIP Codes by Closure Impact (2016-2019)'
).project(
    type='albersUsa'
)

TX_Closure_Impact.display()
```
